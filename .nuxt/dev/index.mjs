...
const appHead = {"meta":[{"charset":"utf-8"},{"name":"viewport","content":"width=device-width, initial-scale=1"},{"hid":"description","name":"description","content":"Discover premium caffeine products - coffee, energy drinks, and more."}],"link":[],"style":[],"script":[],"noscript":[],"title":"Caffeine Products Store"};
...
const _lazy_KjNs0J = () => Promise.resolve().then(function () { return _slug__get$5; });
const _lazy_xl8DTA = () => Promise.resolve().then(function () { return products_get$3; });
const _lazy_PIBOhY = () => Promise.resolve().then(function () { return index_get$1; });
const _lazy_PR0nkN = () => Promise.resolve().then(function () { return products_get$1; });
const _lazy_YZDJkD = () => Promise.resolve().then(function () { return _slug__get$3; });
const _lazy_N6wQCR = () => Promise.resolve().then(function () { return _slug__get$1; });
const _lazy_EaENkd = () => Promise.resolve().then(function () { return search_post$1; });
const _lazy_kdwqQN = () => Promise.resolve().then(function () { return renderer$1; });

const handlers = [
  { route: '/api/categories/:slug', handler: _lazy_KjNs0J, lazy: true, middleware: false, method: "get" },
  { route: '/api/categories/:slug/products', handler: _lazy_xl8DTA, lazy: true, middleware: false, method: "get" },
  { route: '/api/categories', handler: _lazy_PIBOhY, lazy: true, middleware: false, method: "get" },
  { route: '/api/products', handler: _lazy_PR0nkN, lazy: true, middleware: false, method: "get" },
  { route: '/api/products/:slug', handler: _lazy_YZDJkD, lazy: true, middleware: false, method: "get" },
  { route: '/api/recommendations/:slug', handler: _lazy_N6wQCR, lazy: true, middleware: false, method: "get" },
  { route: '/api/search', handler: _lazy_EaENkd, lazy: true, middleware: false, method: "post" },
...
const _inlineRuntimeConfig = {
  ...
  "public": {
    "supabase": {
      "url": "https://geqokzltfbjkuhewwbxv.supabase.co",
      "key": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImdlcW9remx0ZmJqa3VoZXd3Ynh2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3Mzg2NjQ5MjgsImV4cCI6MjA1NDI0MDkyOH0.fTFZuEOArhKo7scp1gq-osX6lZ-tuRFPHce2pu8SEIY",
      "redirect": true,
      "redirectOptions": {
        "login": "/admin",
        "callback": "/confirm",
...
const _slug__get$5 = /*#__PURE__*/Object.freeze({
  __proto__: null,
...
const products_get$2 = defineEventHandler(async (event) => {
  const slug = event.context.params.slug;
  const client = await serverSupabaseClient(event);
  const { data: category } = await client.from("categories").select("id").eq("slug", slug).single();
  if (!category) {
    throw createError({
      statusCode: 404,
  ...
  const { data, error } = await client.from("products").select("*").eq("category_id", category.id).order("created_at", { ascending: false });
  if (error) {
    throw createError({
      statusCode: 500,
      message: error.message
...
const products_get$3 = /*#__PURE__*/Object.freeze({
  __proto__: null,
...
const _slug__get = defineEventHandler(async (event) => {
  const slug = event.context.params.slug;
  const client = await serverSupabaseClient(event);
  const { data: product } = await client.from("products").select("*").eq("slug", slug).single();
  if (!product) {
    throw createError({
      statusCode: 404,
  ...
  const { data: allProducts } = await client.from("products").select("*").neq("id", product.id);
...
Format your response as a simple list with just the names, one per line.`;
  try {
    ...
    const recommendations = result.choices[0].message.content.split("\n").filter((line) => line.trim()).slice(0, 3).map((name) => name.replace(/^[0-9.-\s]+/, "").trim());
    const { data: recommendedProducts } = await client.from("products").select("*").in("name", recommendations);
    if (!(recommendedProducts == null ? void 0 : recommendedProducts.length)) {
      return allProducts.sort(() => Math.random() - 0.5).slice(0, 3);
    }
...
const search_post = defineEventHandler(async (event) => {
  ...
  try {
    ...
    const query = (_a = body == null ? void 0 : body.query) == null ? void 0 : _a.trim();
    if (!query) {
      return [];
    }
    const client = await serverSupabaseClient(event);
    const { data: products, error: dbError } = await client.from("products").select("*").order("created_at", { ascending: false });
    if (dbError) {
...
Calories: ${p.calories}
Certifications: ${((_a2 = p.certifications) == null ? void 0 : _a2.join(", ")) || "None"}
`;
    }).join("\n")}
...
If no exact matches, use semantic understanding to find the closest matches.`;
    try {
      ...
      const result = await response.json();
      const recommendedNames = result.choices[0].message.content.split("\n").filter((line) => line.trim()).map((name) => name.replace(/^[0-9.-\s]+/, "").trim());
      const results = products.filter((p) => recommendedNames.includes(p.name)).slice(0, 5);
      if (results.length === 0) {
        return performBasicSearch(products, query.toLowerCase());
      }
      return results;
...
function performBasicSearch(products, query) {
  const directMatches = products.filter(
    (p) => p.name.toLowerCase().includes(query) || p.brand.toLowerCase().includes(query) || p.description.toLowerCase().includes(query)
  );
  ...
  const keywordResults = products.filter((product) => {
    ...
    if (query.match(/healthy|health|wellness/)) {
      return sugar_content <= 5 && calories <= 10 && (certifications == null ? undefined : certifications.some(
        (c) => ["organic", "natural", "vitamin"].some(
          (term) => c.toLowerCase().includes(term)
        )
      ));
    }
    if (query.includes("organic")) {
      return certifications == null ? undefined : certifications.some(
        (c) => c.toLowerCase().includes("organic")
      );
    }
    if (query.includes("natural")) {
      return certifications == null ? undefined : certifications.some(
        (c) => c.toLowerCase().includes("natural")
      );
    }
    return false;
  });
